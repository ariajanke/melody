...
import { AstExpressionCollector } from '../../src/ast_build/ast_expression_build/ast_expression_collector';
...
describeNamed({ AstExpressionCollector }, () => {
  ...
  const makeInst = (mixed: Readonly<(AstNode | Token)[]>) =>
  ...
  function callsFromInst(inst: () => AstExpressionCollector): string[] {
    return AstHelpers.callsFromInst(inst().finish().node);
  }
