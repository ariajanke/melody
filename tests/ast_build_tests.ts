...
describeNamed({ AstBuild }, () => {
  const makeInstFromStrings = (strs: Readonly<string[]>): () => AstBuild =>
    memoize(() => AstBuild.make(strs.map(makeToken)));
  const callsFromNode = AstHelpers.callsFromInst;
  const idsFromNode = AstHelpers.identifiersFromInst;
  ...
  function buildAstFor(tokens: Readonly<Token[]>): AstNode {
    const inst = AstBuild.make(tokens);
    const res = inst.node();
    if (!res) {
  ...
  function makeBuildAst(tokens: () => Token[]) {
    return (): AstNode => buildAstFor(tokens());
  }
  ...
  describe('builds a mutli-line ast', () => {
    ...
    it('builds two function calls', () => {
      ...
      const visitor = makeVisitor({
        ...makeDefaultVisitor(),
        visitCall(_0: Token, _1: AstNode, args: AstNode): void {
    ...
    it('builds ast with multiple lines end on an unary expression', () => {
      const inst = makeInstFromStrings(['a', '\n', ...notALine]);
      const calls = callsFromNode(inst().node);
      expect(calls).toEqual(['not', 'and']);
    });
    ...
    it('builds ast with multiple lines of unary operators', () => {
      ...
      const calls = callsFromNode(inst().node);
      expect(calls).toEqual(['let', 'not', 'and']);
    });
  });

  function includeAllNIdentifiers(astRes: () => AstNode, identifiers: string[]): void {
  ...
  describe('a + b + c', () => {
    ...
    const buildAst = (): AstNode => buildAstFor(tokens);
    ...
    it('includes two operators', () => {
      ...
      const visitor = makeVisitor({
        ...makeDefaultVisitor(),
        visitCall(_0: Token, receiver: AstNode, args: AstNode): void {
          hitsAtExactly(2);
          receiver.visit(visitor);
          args.visit(visitor);
        }
  ...
  describe('\\naskString()', () => {
    ...
    it('builds single function call node', () => {
      ...
      const visitor = makeVisitor({
        ...makeDefaultVisitor(),
        visitCall(callName: Token, _1: AstNode, _2: AstNode): void {
          hitsAtExactly(1);
  ...
  describe('puts(a, b)\\n', () => {
    ...
    it('creates a function node', () => {
      ...
      const visitor = makeVisitor({
        ...makeDefaultVisitor(),
        visitCall(_0: Token, _1: AstNode, _2: AstNode): void {
          hitsAtExactly(1);
        }
      });
  ...
  describe('with function blocks', () => {
    ...
    it('has one reference to variable "a"', () => {
      let aCount = 0;
      const rootNode = buildAst();
      const visitor = makeVisitor({
        ...makeDefaultVisitor(),
    ...
    it('queue call is outside the function definition', () => {
      ...
      expect(identifiers).toEqual(['<context>', '<context>', 'a']);
    });
  });
  ...
  describe('single line ast', () => {
    ...
    it('puts(askString())', () => {
      ...
      const visitor = makeVisitor({
        ...makeDefaultVisitor(),
        visitCall(callName: Token, _1: AstNode, args: AstNode): void {
          hitsAtExactly(2);
    ...
    it('puts(2 + 3, 5 + 9)', () => {
      ...
      const visitor = makeVisitor({
        ...
        visitLiteral(_0: Token, type: AstLiteralType): void {
          if (type !== 'number')
            { return; }
