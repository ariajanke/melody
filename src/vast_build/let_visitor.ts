...
const DependeeNameRetrieval = (() => {
  function make(): AstNodeVisitor<string[]> {
    const inst = freeze({
      visitFunctionCall(callNode: AstFunctionCallNode, receiver: AstNode, fArgs: AstTupleNode): string[] {
        return [...fArgs.map((node: AstNode) => node.visit(inst))] as unknown as string[];
      },
      ...
      visitTuple: (tuple: AstTupleNode): string[] => {
        return [...tuple.map((node: AstNode) => node.visit(inst))] as unknown as string[];
      },
